...
func NewMemoryScorer(config *config.MemoryConfig) *MemoryScorer {
	return &MemoryScorer{
		config: config,
	}
}
...
func (ms *MemoryScorer) ScoreMemory(memory *models.MemoryEntry) models.MemoryScore {
	now := time.Now()
...
func (ms *MemoryScorer) UpdateMemoryAccess(memory *models.MemoryEntry) {
	now := time.Now()
...
func (ms *MemoryScorer) ScoreMemories(memories []*models.MemoryEntry) {
...
// Used by memory processor to select most valuable memories for processing
func (ms *MemoryScorer) GetTopScoredMemories(memories []*models.MemoryEntry, limit int) []*models.MemoryEntry {
	// Score all memories first
	ms.ScoreMemories(memories)

	// Create copy for sorting to avoid modifying original slice
	sortedMemories := make([]*models.MemoryEntry, len(memories))
